...
> language first meet, and it is the vision tower inside almost every VLM you will be
> asked about. Interviewers use it to test three things at once: whether you can derive a
...
Let $V \in \R^{B\times d_e}$ and $T \in \R^{B\times d_e}$ have unit rows and $S = VT^\top/\tau$. The
image-to-text term treats row $i$ as logits over the $B$ captions with the correct label $i$:
...
$f^\star(v,t) \propto p(t\mid v)/p(t)$ (a standard result: the Bayes-optimal classifier
for "which of the $B$ captions is the true one" scores each by the density ratio). Plugging
that in,

$$
L^\star = -\E\log\frac{\frac{p(t_i\mid v_i)}{p(t_i)}}{\sum_j \frac{p(t_j\mid v_i)}{p(t_j)}}
= \E\log\Big[1 + \frac{p(t_i)}{p(t_i\mid v_i)}\sum_{j\ne i}\frac{p(t_j\mid v_i)}{p(t_j)}\Big]
\;\ge\; \E\log\Big[\frac{p(t_i)}{p(t_i\mid v_i)}(B-1)\Big] \cdot \text{(in expectation)}
$$

and since the negatives are independent of $v_i$, $\E_{t_j}\big[p(t_j\mid v_i)/p(t_j)\big] = 1$, so
by Jensen $L^\star \ge \log B - I(v; t)$, i.e.

$$
\boxed{\;I(v; t) \;\ge\; \log B - L_{\text{InfoNCE}}\;}
$$

*What it means:* minimising the loss maximises a lower bound on the mutual information
between image and text representations, and the bound can never exceed $\log B$. With
$B = 32{,}768$, $\log B \approx 10.4$ nats; with $B = 256$, $5.5$ nats. This is one
mechanism (not the only one) by which batch size acts as a capacity limit on what
contrastive training can learn: if the true MI exceeds $\log B$, the loss saturates
before the representation is finished. (Full derivation in the CPC paper; the
...
positive and the optimiser works in log space; $s$ is initialised to $\log(1/0.07) \approx 2.66$
and clamped so that $1/\tau \le 100$. Why learn it: the right sharpness depends on how
separated the embeddings currently are, and it rises during training as positives pull
...
$t' = e^{s}$ and bias $b$ and labels $z_{ij} = +1$ on the diagonal, $-1$ elsewhere,
...
`(B, 1, 1, T)` so padded positions receive no attention, and a masked mean pool. The
projection heads are bias-free linear maps to a shared $d_e$; normalisation happens after
...
**How you'd test it.** `tests/test_multimodal_clip.py`: (i) `clip_loss` of an all-zero
logit matrix is exactly $\log B$; (ii) `siglip_loss` matches a three-line manual loop over
pairs; (iii) training mini-CLIP for 80 steps on a toy dataset (class $k$ = bright quadrant
$k$, caption = token $k$) with batches of one image per class reaches $> 90\%$ zero-shot
accuracy, and the logit scale respects the clamp; (iv) the same with the SigLIP loss;
(v) the text encoder's output changes when a non-pad token is added but padding is masked.
...
memory: CLIP-scale batches (32k) at ViT-L need hundreds of accelerators purely to hold
the activations of the encoders; the loss is not the bottleneck.
...
DataComp fixed the pool and the training recipe and varied only the filtering, showing
that filtering choices move ImageNet zero-shot by tens of points at the same compute.
The trap in "filter with CLIP": the new model inherits the old model's blind spots (and
its preference for image-text pairs CLIP already understands). MetaCLIP reproduces CLIP's
original *query-balanced* curation without a model in the loop for that reason.

**Failure modes (with the diagnosis).**

* *Bag-of-words behaviour.* CLIP scores "a horse eating grass" and "grass eating a horse"
  nearly equally; the ARO benchmark shows near-chance accuracy on attribute binding and
  relation ordering. Cause: captions rarely contain word-order minimal pairs, so nothing in
...
!!! production "Google — SigLIP as the vision tower for PaliGemma and Gemma-family VLMs"
    ...
    captioning, VQA and OCR-heavy tasks at equal size, and chooses a 400M-parameter
    "shape-optimised" ViT to balance the 2B LLM. Sources: *Sigmoid Loss for Language Image
...
!!! production "Stability AI / CompVis and LAION — open CLIP as text conditioning for Stable Diffusion"
    ...
    usable text-to-image control without training a language model; the cost is inheriting
    CLIP's 77-token limit and composition weaknesses. LAION-5B itself was built by
    filtering CommonCrawl pairs with a CLIP score. Sources: *High-Resolution Image
    Synthesis with Latent Diffusion Models*, Rombach et al., CVPR 2022 (arXiv:2112.10752);
...
!!! production "Pinterest — unified visual embeddings and multimodal search retrieval"
    ...
    *Learning a Unified Embedding for Visual Search at Pinterest*, Zhai et al., KDD 2019
    (arXiv:1908.01707); *OmniSearchSage: Multi-Task Multi-Entity Embeddings for Pinterest
    Search*, Agarwal et al., 2024 (arXiv:2404.16260). See the
...
!!! production "Meta (FAIR) — ImageBind extends the CLIP space to six modalities"
    ...
    modality, text-paired data does not; the cost is that cross-modal alignment quality is
    bounded by the image bridge. Source: *ImageBind: One Embedding Space To Bind Them
...
!!! interview "You built a zero-shot moderation classifier on CLIP. How does it get attacked, and what do you do?"
    ...
    only the relation differs). Mitigations: OCR-and-inpaint text regions before
    embedding; ensemble prompts; use CLIP as a *retrieval* stage that routes to a VLM or a
    supervised classifier for the decision; monitor the score distribution per modality
...
!!! interview "What is the modality gap and where does it bite a system?"
    ...
    whenever you *threshold* similarity across modality pairs, or mix image and text
    ...
    ??? success "Solution"
        ...
        def ensembled_classifier(model, tokenize, templates, class_names):
            ...
            for name in class_names:
                ...
                t = model.proj_txt(model.text_encoder(ids))                       # (n_templ, d_e) un-normalised
...
4. ★★ Derive the gradient of the SigLIP loss with respect to the logit $\ell_{ij}$ and
   explain the role of the $-10$ bias at initialisation.
    ...
    ??? success "Solution"
        $\partial L/\partial \ell_{ij} = -\frac{1}{B} z_{ij}\,\sigma(-z_{ij}\ell_{ij})$. For a negative
        ($z = -1$), $\partial L/\partial\ell = \frac1B\sigma(\ell)$; with $\ell \approx t' v^\top t + b \approx 10\cdot 0 - 10$,
        $\sigma(-10) \approx 4.5\times10^{-5}$, so the $B^2 - B$ negatives contribute almost nothing.
...
5. ★★★ (coding) Implement a hard-negative-aware variant: for each batch, generate one
   ...
   and check that only the variant with hard negatives distinguishes `[red, cube]` from
   `[cube, red]`.

    ??? success "Solution"
        Build `ids_neg = ids[:, [0, 2, 1]]` (swap positions 1 and 2), compute
        `t_neg = model.encode_text(ids_neg)` and logits `(B, 2B)` by concatenating
        `v @ t.T` and `v @ t_neg.T`; the labels are still `arange(B)`. The baseline model
        can solve the batch by matching the *set* of tokens; the variant must encode order
        because the swapped caption shares the set. Evaluate with an order-sensitive
        probe: accuracy at picking the correctly ordered caption from the pair. This is
