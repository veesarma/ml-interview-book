...
**The approach.** Lyft's 2018 series on experimentation in a ridesharing marketplace sets out why user-level randomisation is biased when treatment leaks through shared supply, and what switchback and time-based designs buy you; Uber's 2018 post on its experimentation platform describes the staged-rollout and sequential-testing machinery behind its marketplace experiments. DoorDash's 2018 post describes switchbacks over region-time units for dispatch and pricing changes, and its 2019 post argues that analysis must use cluster-robust standard errors because observations within a switchback unit are correlated; later posts discuss variance reduction and the choice of window length. Cross-link: [experimentation](../part17-ml-system-design/11-notifications-uplift-experimentation.md).
...
!!! tip "How to say it in the interview"
    "For Eats or DoorDash discovery I'd first restrict candidates by deliverable radius and open hours, then retrieve with graph-based or two-tower embeddings (Uber's 2019 post used a GraphSAGE-style model over users, restaurants and dishes and its 2023 post describes two-tower retrieval) and rank with a multi-task model on click, add-to-cart and order. For search I'd follow DoorDash's 2020 'things not strings' post and map queries to entities in a dish and cuisine graph rather than matching strings, because 'ramen' should recall restaurants that never wrote the word. I'd reject reusing a global recsys unchanged; geography is the dominant constraint. Evaluation: order rate per session and null-result rate for search."
...
!!! interview "4. Run an experiment for a new surge pricing algorithm."
    **Sketch.** Switchback over city-time units, cluster-robust SEs, A/A validation, guardrails on driver earnings and rider cancellations. Cross-link: [experimentation](../part17-ml-system-design/11-notifications-uplift-experimentation.md).
    ...
    !!! tip "How to say it in the interview"
        "I'd run a switchback across region-time units because pricing shifts supply between riders, as Lyft's 2018 marketplace experimentation series explains, and analyse with cluster-robust standard errors following DoorDash's 2019 post. I'd validate the design with A/A tests first. I'd reject a user-level A/B. Evaluation: completed trips, driver earnings and cancellation rate with confidence intervals from the cluster-robust estimator."
...
* Uber Engineering, "Meet Michelangelo: Uber's Machine Learning Platform", 2017 ([uber.com](https://www.uber.com/us/en/blog/michelangelo-machine-learning-platform/)); "Scaling Machine Learning at Uber with Michelangelo", 2018 ([uber.com](https://www.uber.com/us/en/blog/scaling-michelangelo/)); "From Predictive to Generative: How Michelangelo Accelerates Uber's AI Journey", 2024 ([uber.com](https://www.uber.com/us/en/blog/from-predictive-to-generative-ai/)).
...
* Chamandy, "Experimentation in a Ridesharing Marketplace", Lyft Engineering, 2018 ([eng.lyft.com](https://eng.lyft.com/experimentation-in-a-ridesharing-marketplace-b39db027a66e)), with Greenhall's companion post on estimating network effects ([eng.lyft.com](https://eng.lyft.com/experimentation-in-a-ridesharing-marketplace-36007a8a31f2)); Uber Engineering, "Under the Hood of Uber's Experimentation Platform", 2018 ([uber.com](https://www.uber.com/us/en/blog/xp/)).
* Uber Engineering, "Forecasting at Uber: An Introduction", 2018 ([uber.com](https://www.uber.com/us/en/blog/forecasting-introduction/)); "Engineering Extreme Event Forecasting at Uber with Recurrent Neural Networks", 2017 ([uber.com](https://www.uber.com/us/en/blog/neural-networks/)).
* Uber Engineering, "Project RADAR: Intelligent Early Fraud Detection System with Humans in the Loop", 2022. [uber.com](https://www.uber.com/us/en/blog/project-radar-intelligent-early-fraud-detection/)
* Uber Engineering, "Food Discovery with Uber Eats: Using Graph Learning to Power Recommendations", 2019 ([uber.com](https://www.uber.com/us/en/blog/uber-eats-graph-learning/)); "Innovative Recommendation Applications Using Two Tower Embeddings at Uber", 2023 ([uber.com](https://www.uber.com/us/en/blog/innovative-recommendation-applications-using-two-tower-embeddings/)).
...
* Uber Newsroom, "Selfies and Security" (Real-Time ID Check), 2016 ([newsroom.uber.com](https://newsroom.uber.com/securityselfies/)); Uber Help, "Identity Verification Checks" ([help.uber.com](https://help.uber.com/en/driving-and-delivering/article/identity-verification-checks)).

**DoorDash**

* DoorDash Engineering, "Next-Generation Optimization for Dasher Dispatch at DoorDash", 2020 ([doordash.engineering](https://doordash.engineering/2020/02/28/next-generation-optimization-for-dasher-dispatch-at-doordash/)); "Using ML and Optimization to Solve DoorDash's Dispatch Problem", 2021 ([careersatdoordash.com](https://careersatdoordash.com/blog/using-ml-and-optimization-to-solve-doordashs-dispatch-problem/)).
* DoorDash Engineering, "Switchback Tests and Randomized Experimentation Under Network Effects at DoorDash", 2018 ([careersatdoordash.com](https://careersatdoordash.com/blog/switchback-tests-and-randomized-experimentation-under-network-effects-at-doordash/)); "Analyzing Switchback Experiments by Cluster Robust Standard Error to Prevent False Positive Results", 2019 ([careersatdoordash.com](https://careersatdoordash.com/blog/cluster-robust-standard-error-in-switchback-experiments/)); "Balancing Network Effects, Learning Effects, and Power in Experiments" ([careersatdoordash.com](https://careersatdoordash.com/blog/balancing-network-effects-learning-effects-and-power-in-experiments/)).
* DoorDash Engineering, "Meet Sibyl: DoorDash's New Prediction Service", 2020 ([careersatdoordash.com](https://careersatdoordash.com/blog/doordashs-new-prediction-service/)); "Building a Gigascale ML Feature Store with Redis", 2020 ([doordash.engineering](https://doordash.engineering/2020/11/19/building-a-gigascale-ml-feature-store-with-redis/)); "Building a Declarative Real-Time Feature Engineering Framework" (Riviera), 2021 ([careersatdoordash.com](https://careersatdoordash.com/blog/building-a-declarative-real-time-feature-engineering-framework/)); "Introducing Fabricator: A Declarative Feature Engineering Framework", 2022 ([careersatdoordash.com](https://careersatdoordash.com/blog/introducing-fabricator-a-declarative-feature-engineering-framework/)).
* DoorDash Engineering, "Things Not Strings: Understanding Search Intent with Better Recall", December 2020. [doordash.engineering](https://doordash.engineering/2020/12/15/understanding-search-intent-with-better-recall/)
* DoorDash Engineering, "Improving ETA Prediction Accuracy for Long-tail Events", 2023 ([careersatdoordash.com](https://careersatdoordash.com/blog/improving-eta-prediction-accuracy-for-long-tail-events/)); "Improving ETAs with multi-task models, deep learning, and probabilistic forecasts", 2024 ([careersatdoordash.com](https://careersatdoordash.com/blog/improving-etas-with-multi-task-models-deep-learning-and-probabilistic-forecasts/)); "Path to high-quality LLM-based Dasher support automation", 2024 ([careersatdoordash.com](https://careersatdoordash.com/blog/large-language-modules-based-dasher-support-automation/)).
