...
def test_rollout_and_relabel_uses_expert_labels():
    ...
    assert obs.shape[0] == labels.shape[0] and ret < 20  # steering hard left crashes
...
def test_bc_fails_and_dagger_recovers():
    ...
    assert dagger_ret > bc_ret + 6 and dagger_ret > expert_ret - 6, (bc_ret, dagger_ret, expert_ret)
