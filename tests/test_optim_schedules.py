...
def test_warmup_cosine_endpoints_and_torch_reference():
    ...
    # Compare the post-warmup part to torch CosineAnnealingLR.
...
def test_wsd_shape():
    ...
    assert f(91) < 1.0 and np.isclose(f(100), 0.1) and np.isclose(f(1000), 0.1)


def test_inverse_sqrt_peak_at_warmup():
