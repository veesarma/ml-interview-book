...
def test_pca_eig_and_svd_agree_and_match_numpy_svd():
    X = _data()
    V_e, var_e, mu = pca.pca_eig(X, 2)
    V_s, var_s, _ = pca.pca_svd(X, 2)
    ...
    np.testing.assert_allclose(var_s, s[:2] ** 2 / (X.shape[0] - 1))
    # projections agree with U S
    Z = pca.transform(X, V_s, mu)
    assert Z.shape == (300, 2)
    np.testing.assert_allclose(np.abs(Z), np.abs((X - X.mean(0)) @ Vt[:2].T), atol=1e-8)


def test_reconstruction_error_equals_dropped_variance_and_whitening():
    X = _data()
    N = X.shape[0]
    V, var, mu = pca.pca_svd(X, 2)
    X_hat = pca.inverse_transform(pca.transform(X, V, mu), V, mu)
    total = np.trace(np.cov(X.T))
    dropped = total - var.sum()
    np.testing.assert_allclose(((X - X_hat) ** 2).sum() / (N - 1), dropped, atol=1e-8)
    ratio = pca.explained_variance_ratio(var, total)
...
def test_randomized_svd_close_to_exact():
    ...
    A5 = U @ np.diag(s) @ Vt
    A5_ref = np.linalg.svd(A, full_matrices=False)
    A5_ref = A5_ref[0][:, :5] @ np.diag(A5_ref[1][:5]) @ A5_ref[2][:5]
